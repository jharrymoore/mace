...
class WeightedEnergyForcesChargesLoss(torch.nn.Module):
    ...
    def forward(self, ref: Batch, pred: TensorDict) -> torch.Tensor:
        return (
            ...
            + self.forces_weight * mean_squared_error_forces(ref, pred)
            + self.charges_weight * weighted_mean_squared_error_charge(ref, pred)
        )
...
class WeightedEnergyForcesDipolesChargesLoss(torch.nn.Module):
    def __init__(
        self, energy_weight=1.0, forces_weight=1.0, charges_weight=1.0, dipole_weight=1.0
    ) -> None:
        super().__init__()
        self.register_buffer(
            "energy_weight",
